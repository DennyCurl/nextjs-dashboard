...
Система інтегрована з існуючою таблицею `drug_administrations` для автоматичного збереження інформації про видані медикаменти пацієнтам під час медичних візитів.

## Структура таблиці `drug_administrations`

```sql
CREATE TABLE public.drug_administrations (
  id bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  id_drug bigint NULL,                    -- Посилання на drugBase.id
  ...
  CONSTRAINT drug_administrations_id_drug_fkey FOREIGN KEY (id_drug) REFERENCES "drugBase" (id) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT drug_administrations_id_visit_fkey FOREIGN KEY (id_visit) REFERENCES visits (id) ON UPDATE CASCADE ON DELETE CASCADE
);
```
...
3. **Транзакція БД**: В рамках однієї транзакції створюється:
   ...
   - **Записи видачі медикаментів (`drug_administrations`)**
